...
elseif ($article_id == 0)
{
	$sql = 'SELECT a.article_id ' .
            'FROM ' .$GLOBALS['ecs']->table('article'). ' AS a, '  .$GLOBALS['ecs']->table('article_cat'). ' AS c ' .
            ' WHERE a.cat_id = c.cat_id and c.cat_type = 5 AND a.is_open = 1 ' .
            'ORDER BY c.sort_order ASC, a.article_id limit 0,1';
	$article_id=$db->getOne($sql);
}
