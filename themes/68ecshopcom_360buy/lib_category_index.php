...
{ 
$sql =  'SELECT g.goods_id, g.goods_name,g.act_title, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, '. 
"IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ". 
'promote_start_date, promote_end_date, g.act_title, g.goods_thumb, goods_img '. 
...
$goods[$idx]['promote_price'] = $promote_price >0 ?price_format($row['promote_price']) : ''; 
$goods[$idx]['shop_price']   = price_format($row['promote_price']); 
} 
else 
{ 
...
$goods[$idx]['act_title']           = $row['act_title']; 

$goods[$idx]['goods_name']         = $row['goods_name']; 
$goods[$idx]['brief']        = $row['act_title']; 
...
function get_pro_top10($cats)
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name,g.act_title, g.shop_price, g.goods_thumb, SUM(og.goods_number) as goods_number ' .
           'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g, ' .
    ...
    for ($i = 0, $count = count($arr); $i < $count; $i++)
    {
        ...
        $arr[$i]['price'] = price_format($arr[$i]['shop_price']);
		 $arr[$i]['act_title'] = $arr[$i]['act_title'];
