...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
	...
	{
	$brands_www68ecshopCom=array();
	$brands_www68ecshopCom[0] = $brands[0];
	$brands_qq=explode(',' , $cat['brand_qq']);
	foreach($brands_qq as $key_qq => $brand_qq)
	{
		foreach($brands as $key_www68ecshopcom=>$brand_68ecshopcom)
		{
			if($brand_68ecshopcom['brand_name']==$brand_qq)
			{
				$brands_www68ecshopCom[]=$brand_68ecshopcom;
				break;
			}
		}
	}
	$brands=$brands_www68ecshopCom;
	$smarty->assign('brands', $brands);
	}
    ...
    if ($cat['filter_attr'] > 0)
    {
	 ...
	 if($cat['attr_www68ecshopcom'])
	 {
		 $attr_www68ecshopcom=explode("\r\n", $cat['attr_www68ecshopcom']);
		 $attr_def_www68ecshopcom=array();
		 foreach($attr_www68ecshopcom as $attr_68ecshopcom)
		 {
			if ($attr_68ecshopcom)
			 {
				    $attr_qq = explode(":", $attr_68ecshopcom);
					$attr_def_www68ecshopcom[$attr_qq[0]]=explode(",", $attr_qq[1]);
			}
		 }

		//echo '<pre>';
		//print_r($attr_def_www68ecshopcom);
		//echo '</pre>';


		foreach ($all_attr_list as $key_attr_qq => $attr_old_68ecshopcom)
		{
			foreach($attr_def_www68ecshopcom as $key_def_68ecshopcom => $attr_def_68ecshopcom)
			{
				if ( $attr_old_68ecshopcom['filter_attr_name'] == $key_def_68ecshopcom)
				{
					 $attr_list_qq = array();
					 $attr_list_qq[0]= $attr_old_68ecshopcom['attr_list'][0];
                     foreach($attr_def_68ecshopcom as  $attr_new_68ecshopcom)
					 {
							foreach($attr_old_68ecshopcom['attr_list'] as $attr_temp_qq)
						    {
								if($attr_temp_qq['attr_value'] == $attr_new_68ecshopcom )
								{
...
{
    return $GLOBALS['db']->getRow('SELECT cat_name, keywords, cat_desc, style, grade, filter_attr, parent_id, brand_qq, attr_www68ecshopcom FROM ' . $GLOBALS['ecs']->table('category') .
        " WHERE cat_id = '$cat_id'");   //前面这行代码有修改   By www.68ecshop.com
}
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id,g.click_count,g.act_title, g.goods_number,g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
