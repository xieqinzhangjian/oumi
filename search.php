...
else
{
	...
	/* 代码添加_START   By  www.68ecshop.com  */	
    if ($page == 1 && $Recordkw && $count) Recordkeyword($Recordkw, $count, 'ecshop');
	if($count==0)
	{
	...
	{
		$smarty->assign('beizhuxinxi_www_68ecshop_com', '关键词<font color=#ff3300>'.$_REQUEST['keyword_zm'].'</font>搜索结果为零，<br>但是我们为您匹配到了相关关键词<font color=#ff3300>'.$_REQUEST['keywords'].'</font>，下面是它的查询结果！');
	}
	/* 代码添加_END   By  www.68ecshop.com  */
    ...
    /* 查询商品 */
    $sql = "SELECT g.goods_id, g.goods_name,g.act_title, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		{
				//$replacet[$key_www_68ecshop_com]=  '<strong style="color:#ff3300;">'. $temp_qq .'</strong>';;
				$arr[$row['goods_id']]['goods_name_www_68ecshop_com'] = preg_replace('/(?!<[^>]*)'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#ff3300;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['goods_name_www_68ecshop_com']);
		}
		/* 代码添加_END  By  www.68ecshop.com */
