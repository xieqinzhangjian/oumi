...
function get_top10($cats = '')
{
    ...
    for ($i = 0, $count = count($arr); $i < $count; $i++)
    {
		...
		$arr[$i]['count']   	 = selled_count($arr[$i]['goods_id']);
    }
...
function get_promote_goods($cats = '')
{
	...
	/*yyy 修改131104start*/
		...
		{
	$goods[$idx]['zhekou'] = (number_format(intval($row['promote_price'])/intval($row['shop_price']),2))*10;
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style,g.act_title, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img, b.brand_name ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price,g.is_shipping,g.act_title, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
