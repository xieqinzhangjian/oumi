...
class SplitWord
{
    ...
    //附加词典
    var $addonDic = array();
    var $addonDicFile = 'includes/codetable/words_addons.dic';
    ...
    var $mainDicFile = 'includes/codetable/base_dic_full.dic';
    //是否直接载入词典（选是载入速度较慢，但解析较快；选否载入较快，但解析较慢，需要时才会载入特定的词条）
    var $mainDicFileZip = 'includes/codetable/base_dic_full.zip';
    var $isLoadAll = FALSE;
    ...
    function __construct($source_charset='utf-8', $target_charset='utf-8', $load_all=TRUE, $source='')
    {
        ...
        if(file_exists(ROOT_PATH .$this->mainDicFile)) $this->isUnpacked = TRUE;
    ...
    */
    ...
    function LoadDict( $maindic='' )
    {
		...
		$this->mainDicFileZip = ROOT_PATH . $this->mainDicFileZip;
