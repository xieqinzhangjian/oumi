...
$sql_attr_www_ecshop120_com="SELECT a.attr_id, ga.goods_attr_id FROM ". $GLOBALS['ecs']->table('attribute') . " AS a left join ". $GLOBALS['ecs']->table('goods_attr') . "  AS ga on a.attr_id=ga.attr_id  WHERE a.is_attr_gallery=1 and ga.goods_id='" . $goods_id. "' order by ga.goods_attr_id ";
$goods_attr=$GLOBALS['db']->getRow($sql_attr_www_ecshop120_com);
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'get_gallery_attr')
{
	...
	$gallery_list_www_ecshop120_com=get_goods_gallery_attr_www_ecshop120_com($goods_id, $goods_attr_id);
	$gallery_content=array();
	$gallery_content['thumblist'] ='<ul>';
	foreach($gallery_list_www_ecshop120_com as $gkey=>$gval)
	{
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
	...
	$smarty->assign('evaluation', get_evaluation_sum($goods_id));
    ...
    else
    {
		...
		$smarty->assign('pictures',            get_goods_gallery_attr_www_ecshop120_com($goods_id, $goods_attr_id)); // 商品相册_修改 By www.ecshop120.com
...
$smarty->assign('xml',get_graph($goods_id));
$smarty->display('goods.dwt',      $cache_id);

function get_graph($goods_id)
{
	...
	$general_xml = "<graph caption='最近30天价格走势' lineThickness='1' showValues='0' formatNumberScale='0' anchorRadius='2'   divLineAlpha='20' divLineColor='CC3300' divLineIsDashed='1' showAlternateHGridColor='1' alternateHGridAlpha='5' alternateHGridColor='CC3300' shadowAlpha='40' labelStep='2' numvdivlines='5' chartRightMargin='35' bgColor='FFFFFF,CC3300' bgAngle='270' bgAlpha='10,10' outCnvBaseFontSize='12' baseFontSize='12' >";
        foreach($start_date_arr AS $k => $val)
        {

            $seriesName = local_date('Y-m', $start_date_arr[$k]);
            $general_xml .= "<dataset seriesName='$seriesName' color='" . display_color($k) . "' anchorBorderColor='" . display_color($k+1) . "' anchorBgColor='" . display_color($k+2) . "'>";
			$count = get_count_by_goods_id($goods_id);
			if($count == 0)
			{
				 $lastDay = 0;
				 $day = gmdate('d',time() +  $timezone * 3600);
				 if ($lastDay == 0)
                 {
					$price = get_price_by_goods_id($goods_id);
                    $lastDay++;
                    for (; $lastDay <= $day; $lastDay++)
                    {
                        $general_xml .= "<set value='$price' />";
                    }
                }
			}
			else
			{
            	$sql = "SELECT FLOOR((add_time - $start_date_arr[$k]) / (24 * 3600)) AS sn, add_time, price".
                " FROM " .$GLOBALS['ecs']->table('test').
                " WHERE goods_id = '$goods_id' AND add_time >= '$start_date_arr[$k]' AND add_time <= " .($end_date_arr[$k] + 86400).
                " GROUP BY sn";
				...
				for($i =1; $i<=$day1; $i++)
				{
					...
					$c = strtotime($b."-".$i)+86400;
					$s = "select t.* from ".$GLOBALS['ecs']->table('test')." as t where goods_id = '$goods_id' and t.add_time <= ".$c." order by t.add_time desc limit 1";
					$r = $GLOBALS['db']->getRow($s);
					if($r != '')
					{
						$general_xml .= "<set value='$r[price]' />";
						$old_price = $r['price'];
					}
					else
					{
						if($old_price == '')
						{
							$s = "select t.* from ".$GLOBALS['ecs']->table('test')." as t where goods_id = '$goods_id' order by t.add_time asc limit 1";
							$r = $GLOBALS['db']->getRow($s);
							$general_xml .= "<set value='$r[price]' />";
						}
						else
						{
							$general_xml  .= "<set value='$old_price' />";
						}
					}
				}
        }
...
function get_count_by_goods_id($goods_id)
{
	$sql = "select count(*) from ".$GLOBALS['ecs']->table('test')." where goods_id = '$goods_id'";
	return $GLOBALS['db']->getOne($sql);
}

function get_price_by_goods_id($goods_id)
{
	$sql = "select shop_price from ".$GLOBALS['ecs']->table('goods')." where goods_id = '$goods_id'";
	return $GLOBALS['db']->getOne($sql);
}
function display_color($n)
{
    /* 随机显示颜色代码 */
 ...
 * @return  array
 */
function get_goods_gallery_attr_www_ecshop120_com($goods_id, $goods_attr_id)
{
